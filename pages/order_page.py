...
class OrderPageScooter:
    ...
    def fill_surname(self, surname):
        self.driver.find_element(*self.locators.SURNAME).send_keys(surname)

    def fill_address(self, address):
        self.driver.find_element(*self.locators.ADDRESS).send_keys(address)

    def fill_metro(self, metro):
        self.driver.find_element(*self.locators.METRO).click()
    ...
    def click_order_next(self):
        self.driver.find_element(*self.locators.NEXT).click()

    def fill_date(self, date):
        self.driver.find_element(*self.locators.DATE).click()
        self.wait_for_load(self.locators.DATEPICKER)
        self.driver.find_element(*date).click()

    def fill_period(self, period):
        self.driver.find_element(*self.locators.PERIOD).click()
    ...
    def fill_comment(self, comment):
        self.driver.find_element(*self.locators.COMMENT).send_keys(comment)
    ...
    def click_order_window_button(self):
        self.driver.find_element(*self.locators.ORDER_CONFIRM_BUTTON).click()

    def check_order(self):
        assert "Заказ оформлен" in self.driver.find_element(*self.locators.ORDER_SUCCESS).text

    def return_base_page(self):
        self.driver.find_element(*self.locators.BUTTON_SHOW_STATUS).click()
        WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located((self.locators.STATUS)))
        self.driver.find_element(*self.locators.BASE_PAGE_BUTTON).click()
